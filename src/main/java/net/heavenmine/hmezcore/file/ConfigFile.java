...
//        main.getConfig().save(new File(main.getDataFolder(), "config.yml"));
    ...
    public List<HomeAmount> getHomeAmount() {
        ConfigurationSection homeAmount = main.getConfig().getConfigurationSection("home");
        List<HomeAmount> homeAmountList = new ArrayList<>();
        for (String key : homeAmount.getKeys(false)) {
            homeAmountList.add(new HomeAmount(key, homeAmount.getInt(key)));
        }
        return homeAmountList;
    }
}
